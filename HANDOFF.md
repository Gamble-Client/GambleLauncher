...
- Coordinated publication delivered loader `1.4.25` and client build `20260907154631`/version `1.250` while retaining launcher `0.1.133`. Artifact sources: launcher `209aff4`, client handoff anchor `69e4472c` (client code identical to tested `be106f652`), and site API patch `c6104813`; generated site metadata/state commits are `dce110c`/`b03195b`. Later documentation-only commits do not change artifact identity.
...
- Loader `1.4.26` and matching isolated payload build `20260907183425`/`1.250` correct localized Windows account recognition in both private-storage and native-staging checks. Tested source: `f5ce16348f47b63edd2f3a8be3be075fda13d24d`; root evidence: `GambleClient/docs/loader-1.4.26-windows-sid.md` on branch `codex/loader-1426-sid-localization`. Builder/Custom Cursor work was excluded.
- `BB6258E0F5` fingerprints the loader's rejected parent-ACL check. The user's French screenshot shows current user, System and Administrators, with localized names. Trust now uses exact OS-derived SIDs from verified JDK principal classes, never translated display strings. Unsupported providers fail closed; foreign writers, owner checks, signed authorization and provenance remain enforced. No user ACL changes or security bypass are required.
- Actual Windows run `34152170701` passed **153** tests (6 native, 74 baseline, 73 production), zero failures/errors/skips. It exercises a genuinely renamed built-in Administrators group, native staging, private storage, hostile ACL controls and repeated concurrent startup. A separate synthetic System display-name test retains the genuine SID. This is not a fully French OS or authenticated user gameplay test.
- Local **698** tests and the exact five-tier build, three packaging regressions and three loader-platform audits passed. Isolated artifacts: `/tmp/gamble-loader-1426-tiers.2AXuIy`. Canonical live Windows loader independently downloaded: 7,305,092 bytes, SHA-256 `05cb598a04fbd6a771a7f2a53e8568aed99d8f8c61d45fdd95bdac38ed20555f`.
- Site metadata/audit commits `f091ee9`/`ddacd6f`, matching payload metadata `549ecc2`, corrected release-state `acbd3f0`, final Site handoff `301d4ce`; release reports remote byte/hash/provenance audit passed and Pages deployed at `https://c2b287be.gamble-client-b67.pages.dev`. Release-state client source is `f5ce16348f47b63edd2f3a8be3be075fda13d24d`. Loader 1.4.25 remains immutable for rollback.
...
- Release-script corrections shipped in Site commits9262c40/8e3aa76: run lock, safer child/cookie cleanup, receipt validation, fail-closed existing-object checks, current-loader preservation, strict platform/hash manifests, honest Discord failures, mode-specific metadata commit titles and package-input change detection. Independent release fixtures, six malformed manifest probes, process cleanup,238 Site tests, release audit, Pages build, script-copy equality and syntax/diff checks passed. R2 read-before-put is not atomic across hosts. Full report: /home/theac/Documents/Codex/2026-08-26/i-usually-will-have-one-chat-2/outputs/launcher-ux-audit/RELEASE-0.1.134.md. CI/install/render receipts are in /tmp/launcher-134-release.yXSWnU.
